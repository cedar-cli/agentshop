...
test("轻薄本案例在购买历史原位提供实时执行入口", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "出差轻薄本采购" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "运行真实 LLM" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: /实时采购/ })).toHaveCount(0);
});

test("买家端覆盖会话、主动服务与 Inbox", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "我的消费 Agent" }),
  ).toBeVisible();
...
test("家庭补库明确表达无人类采购指令", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "推进到库存触发点" }),
  ).toBeVisible();
  await expect(
    page.getByRole("textbox", { name: "向消费 Agent 描述需求" }),
  ).toBeDisabled();
...
test("假设时间机器：拖偏好后冠军翻盘", async ({ page, viewport }) => {
  test.skip(
    (viewport?.width ?? 0) < 980,
    "决策检视栏在窄屏隐藏（沿用既有响应式设计）",
  );
  await page.goto("/");
...
test("决策剧场：海选决出 3 家 + 议价擂台", async ({ page }) => {
  ...
  await expect(
    page.locator("strong").filter({ hasText: "我的 C-Agent" }),
  ).toBeVisible();
});
...
test("生态演化：交易流驱动信誉 + 诊断建议", async ({ page }) => {
  ...
  await expect(page.getByText(/满意|差评|流失/).first()).toBeVisible({
    timeout: 5000,
  });
});
...
test("意图增长：落选对话训练商品并在下一次竞争中升榜", async ({
  page,
  viewport,
}) => {
  ...
  await expect(
    page.getByText("商品能力从输掉的 Agent 交易中生长"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("闭环完成 · 每笔订单开始下一轮学习"),
  ).toBeVisible();
  await expect(page.getByText("$8,960").first()).toBeVisible();
  await expect(page.getByText("#1").first()).toBeVisible();
  await page.screenshot({
    path: `test-results/intent-growth-${(viewport?.width ?? 0) < 980 ? "mobile" : "desktop"}.png`,
    fullPage: true,
  });
