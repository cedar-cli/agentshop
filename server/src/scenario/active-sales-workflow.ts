...
export const activeSalesProduct: ActiveSalesProductFact = {
  ...
  stock: 1200,
  rawDescription:
    "柔软天然的婴儿床品套装，让宝宝整夜安心。标准配送，退货规则见政策页。",
  facts: [
    "TENCEL Lyocell 68%",
...
export const activeSalesBuyers: ActiveSalesBuyerFact[] = [
  {
    id: "mia",
    displayName: "Mia Park · New Parent",
    consent: "open",
  ...
  {
    id: "daycare",
    displayName: "Little Steps · Daycare Ops",
    consent: "limited",
  ...
  {
    id: "noah",
    displayName: "Noah Kim · Privacy First",
    consent: "closed",
...
function matchReasons(buyerId: string): string[] {
  if (buyerId === "mia")
    return ["低敏证据可验", "$164 在预算内", "72 小时送达"];
...
async function buildPassport(agent?: ActiveSalesLlmAgent) {
  if (!agent)
    return {
      draft: fallbackPassport(activeSalesProduct),
      generatedBy: "fallback" as const,
      fallbackReason: "LLM 未启用",
    };
  try {
    return {
      draft: await agent.buildPassport(activeSalesProduct),
...
async function writeProposal(
  ...
  if (!agent)
    return {
      pitch: fallbackProposal(activeSalesProduct, buyer),
      generatedBy: "fallback" as const,
      fallbackReason: "LLM 未启用",
    };
  try {
    return {
      pitch: await agent.writeProposal(activeSalesProduct, buyer, passport),
      generatedBy: "llm" as const,
    };
  } catch (error) {
    return {
...
export async function runActiveSalesWorkflow(
  ...
  await router.publish({
    transactionId,
    type: "active-sale.product.ingested",
    source: "seller-catalog",
    payload: {
      productId: activeSalesProduct.id,
  ...
  await router.publish({
    transactionId,
    type: "active-sale.passport.published",
    source: "seller-agent",
    payload: {
      productId: activeSalesProduct.id,
      ...passportResult.draft,
      coverageBefore: 58,
      coverageAfter: 92,
      generatedBy: passportResult.generatedBy,
      fallbackReason: passportResult.fallbackReason,
    },
  });

  const proposalResults = new Map(
    await Promise.all(
      activeSalesBuyers
        ...
        .map(
          async (buyer) =>
            [
              buyer.id,
              await writeProposal(buyer, passportResult.draft, agent),
            ] as const,
        ),
    ),
  );
  ...
  for (const buyer of activeSalesBuyers) {
    await router.publish({
      transactionId,
      type: "active-sale.buyer.matched",
      source: "consent-router",
      target: buyer.id,
      payload: {
        buyerId: buyer.id,
        ...
        exposedFields: buyer.exposedFields,
        matchScore: matchScore(buyer.id),
        reasons: matchReasons(buyer.id),
      },
    });
  }
  ...
  for (const buyer of activeSalesBuyers) {
    if (buyer.consent === "closed") {
      await router.publish({
        transactionId,
        type: "active-sale.proposal.blocked",
        source: "consent-router",
        target: buyer.id,
        payload: {
          buyerId: buyer.id,
          ...
          consent: "closed",
          reason:
            "买家关闭卖家主动触达；Router 未向 Seller Agent 暴露任何画像字段。",
          exposedFieldCount: 0,
        },
      });
      continue;
    ...
    if (!proposal)
      throw new Error(`missing active sales proposal for ${buyer.id}`);
    await router.publish({
      transactionId,
      type: "active-sale.proposal.routed",
      source: "seller-agent",
      target: buyer.id,
      payload: {
        buyerId: buyer.id,
  ...
  await router.publish({
    transactionId,
    type: "active-sale.buyer.selected",
    source: "mia-buyer-agent",
    target: "seller-agent",
    payload: {
      buyerId: "mia",
      displayName: "Mia Park · New Parent",
      score: 94,
      reason:
        "可验证低敏证据、72 小时履约和预算匹配共同胜出，价格不是唯一决策因素。",
      comparison: [
        {
          product: "CalmSleep Kit v2.1",
          priceUsd: 164,
          evidence: "verified",
          delivery: "72h",
          score: 94,
  ...
  await router.publish({
    ...
    payload: {
      ...
      displayName: "Mia Park · New Parent",
      productId: activeSalesProduct.id,
      productName: activeSalesProduct.name,
      amountUsd: activeSalesProduct.priceUsd,
      autoApproved: true,
      humanClicks: 0,
    },
  });
