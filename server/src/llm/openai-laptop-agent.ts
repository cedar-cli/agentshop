...
export class OpenAILaptopAgent implements LaptopLlmAgent {
  ...
  async parseIntent(requestText: string) {
    const response = await this.client.responses.parse({
      model: getModel(), reasoning: { effort: "low" }, max_output_tokens: 350,
      input: [
        { role: "system", content: "你是买家采购 Agent。把用户的轻薄本需求提取成结构化采购意图。未明确时采用保守默认值：重量1.3kg、续航12小时、交期72小时、全国联保。四项偏好总和应为100。" },
  ...
  async generateProposal(seller: LaptopSellerFact, intent: LaptopIntent): Promise<LaptopQuoteDraft> {
    const response = await this.client.responses.parse({
      model: getModel(), reasoning: { effort: "low" }, max_output_tokens: 300,
      input: [
        { role: "system", content: "你是轻薄本商家 Agent。只能依据输入事实报价，不得修改重量、续航、交期和保修，不得低于最低价或高于挂牌价。用不超过80字解释报价优势。" },
  ...
  async negotiate(seller: LaptopSellerFact, intent: LaptopIntent, offer: LaptopCounterOffer): Promise<LaptopNegotiationDraft> {
    const response = await this.client.responses.parse({
      model: getModel(), reasoning: { effort: "low" }, max_output_tokens: 300,
      input: [
        { role: "system", content: "你是中标轻薄本商家 Agent。根据买家目标价进行一次让步，最终价不得低于商家底价、不得高于原报价。用不超过80字说明让步与附加权益。" },
        { role: "user", content: JSON.stringify({ intent, seller, counterOffer: offer }) },
      ],
      text: { format: zodTextFormat(negotiationSchema, "laptop_negotiation") },
    }, { signal: AbortSignal.timeout(8_000) });
