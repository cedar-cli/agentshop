...
export interface LaptopLlmAgent {
  parseIntent(
    ...
    Omit<LaptopIntent, "requestText" | "generatedBy" | "fallbackReason">
  >;
  generateProposal(
...
export function validateLaptopQuote(
  ...
  if (draft.quotedPriceCny < seller.minimumPriceCny)
    throw new Error("报价低于卖家底价");
  if (draft.quotedPriceCny > seller.listPriceCny)
    throw new Error("报价高于挂牌价");
  if (
    seller.minimumPriceCny <= intent.budgetCny &&
    draft.quotedPriceCny > intent.budgetCny
  ) {
    throw new Error("报价忽略买家预算");
  }
  return {
    quotedPriceCny: Number(
      Math.max(draft.quotedPriceCny, seller.preferredPriceCny).toFixed(0),
    ),
...
export function validateLaptopNegotiation(
  ...
  if (draft.finalPriceCny < seller.minimumPriceCny)
    throw new Error("议价结果低于卖家底价");
  if (draft.finalPriceCny > offer.originalPriceCny)
    throw new Error("议价后价格反而上涨");
  return {
