...
const restockTimeAdvancedSchema = z.object({
  ...
  beforePercent: z.number().min(0).max(100),
  afterPercent: z.number().min(0).max(100),
  simulated: z.literal(true),
});
const restockInventoryForecastSchema = z.object({
  item: z.string().min(1),
  remainingPercent: z.number().min(0).max(100),
  hoursUntilEmpty: z.number().positive(),
...
const restockAuthorizationCheckSchema = z.object({
  ...
  singlePurchaseLimitCny: z.number().positive(),
  monthlyLimitCny: z.number().positive(),
  monthlyRemainingCny: z.number().min(0),
  cooldownDays: z.number().int().min(0),
  daysSinceLastPurchase: z.number().int().min(0),
...
const restockSellerSelectedSchema = z.object({
  ...
  score: z.number().min(0).max(100),
  reason: z.string().min(1),
  proposal: restockProposalSchema,
});
const restockBundleNegotiatedSchema = z.object({
  ...
  finalPriceCny: z.number().positive(),
  concessionCny: z.number().min(0),
  bundle: z.array(z.string().min(1)).min(1),
...
const restockMemoryUpdatedSchema = z.object({
  consumptionCycleDays: z.number().positive(),
  nextTriggerLeadDays: z.number().positive(),
...
const activeSalesProductIngestedSchema = z.object({
  ...
  rawDescription: z.string().min(1).max(1000),
});
const activeSalesPassportPublishedSchema = z.object({
  ...
  coverageBefore: z.number().min(0).max(100),
  coverageAfter: z.number().min(0).max(100),
  generatedBy: z.enum(["llm", "fallback"]),
  fallbackReason: z.string().max(240).optional(),
});
const activeSalesBuyerMatchedSchema = z.object({
  ...
  consent: z.enum(["open", "limited", "closed"]),
...
const activeSalesProposalBlockedSchema = z.object({
  buyerId: z.string().min(1),
  displayName: z.string().min(1),
  consent: z.literal("closed"),
  reason: z.string().min(1).max(240),
  exposedFieldCount: z.literal(0),
});
...
const demandNeedReceivedSchema = z.object({
  ...
  source: z.enum(["demo-fixture", "consumer-transaction"]),
});
...
const demandMarketAggregatedSchema = z.object({
  ...
  simulatedMarketIntents: z.number().int().positive(),
  clusters: z
    .array(
      z.object({
        label: z.string().min(1),
        sampleHits: z.number().int().min(0),
        simulatedDemand: z.number().int().min(0),
...
const demandProductForecastedSchema = z.object({
  ...
  candidates: z
    .array(
      z.object({
        product: z.string().min(1),
        marketHeat: z.number().min(0).max(100),
        supplyFit: z.number().min(0).max(100),
        marginFit: z.number().min(0).max(100),
...
const demandSupplyNegotiatedSchema = z.object({
  ...
  delayPenaltyPercentPerDay: z.number().min(0).max(100),
...
const demandBatchCompletedSchema = z.object({
  ...
  checks: z.array(z.string().min(1)).min(1),
  productPassportId: z.string().min(1),
  simulated: z.literal(true),
});
...
const distributionAgentMatchedSchema = z.object({
  ...
  authorizedIntentCount: z.number().int().min(0),
  matchScore: z.number().min(0).max(100),
});
const distributionOrdersCompletedSchema = z.object({
  orderCount: z.number().int().positive(),
  gmvUsd: z.number().positive(),
  b2cOrders: z.number().int().min(0),
  b2bOrders: z.number().int().min(0),
  attestedOrders: z.number().int().min(0),
  sampleOrders: z
    .array(
      z.object({
...
const distributionCommissionReleasedSchema = z.object({
  ...
  condition: z.literal("fulfilled-and-attested"),
  hashChainVerified: z.literal(true),
});
...
const intentGrowthMarketRankedSchema = z.object({
  ...
  currentSellerId: z.string().min(1),
...
const intentGrowthSellerShortlistedSchema = z.object({
  intentId: z.string().min(1),
  sellerId: z.string().min(1),
...
const intentGrowthDialogueRoundSchema = z.object({
  buyerId: z.string().min(1),
  buyerName: z.string().min(1),
...
const intentGrowthSellerLostSchema = z.object({
  intentId: z.string().min(1),
  sellerId: z.string().min(1),
  winnerId: z.string().min(1),
  finalRank: z.number().int().positive(),
  reason: z.string().min(1),
  uncoveredFields: z.array(z.string().min(1)).min(1),
});
const intentGrowthLearningStartedSchema = z.object({
  conversationGroups: z.number().int().positive(),
  dialogueRounds: z.number().int().positive(),
  observedSignals: z.number().int().positive(),
  simulatedSignalVolume: z.literal(true),
  generatedBy: z.enum(["llm", "fallback"]),
  fallbackReason: z.string().max(240).optional(),
});
const intentGrowthIntentExtractedSchema = z.object({
  key: z.enum(["wash_temp", "use_context", "wash_cycles", "bulk_sla"]),
  label: z.string().min(1),
  value: z.string().min(1),
  confidence: z.number().min(0).max(100),
  productField: z.string().min(1),
  evidence: z.array(z.string().min(1)).min(2),
  status: z.literal("ready"),
  generatedBy: z.enum(["llm", "fallback"]),
  fallbackReason: z.string().max(240).optional(),
});
const intentGrowthGapDetectedSchema = z.object({
  productId: z.string().min(1),
  coverageBefore: z.number().min(0).max(100),
...
const intentGrowthProductFieldUpdatedSchema = z.object({
  productId: z.string().min(1),
...
const intentGrowthProductVersionPublishedSchema = z.object({
  productId: z.string().min(1),
  productName: z.string().min(1),
  previousVersion: z.literal("v2.1"),
  version: z.literal("v2.2"),
  coverageBefore: z.number().min(0).max(100),
  coverageAfter: z.number().min(0).max(100),
  writtenFields: z.array(z.string().min(1)).min(1),
  summary: z.string().min(1).max(300),
});
const intentGrowthBuyerRematchedSchema = z.object({
  intentId: z.string().min(1),
  buyerName: z.string().min(1),
  quantity: z.number().int().positive(),
  deadlineDays: z.number().positive(),
  ...
  rankBefore: z.number().int().positive(),
  rankAfter: z.number().int().positive(),
  scoreBreakdown: z.object({
    context: z.number().min(0).max(100),
    wash: z.number().min(0).max(100),
    durability: z.number().min(0).max(100),
    bulkSla: z.number().min(0).max(100),
  }),
});
const intentGrowthQuoteRequestedSchema = z.object({
  quoteId: z.string().min(1),
  quantity: z.number().int().positive(),
  budgetUsd: z.number().positive(),
  requestedDeliveryDays: z.number().positive(),
  requestedTerms: z.array(z.string().min(1)).min(1),
});
const intentGrowthTermsNegotiatedSchema = z.object({
  quoteId: z.string().min(1),
  unitPriceUsd: z.number().positive(),
  quantity: z.number().int().positive(),
  deliveryDays: z.number().positive(),
  delayPenaltyPercent: z.number().min(0).max(100),
  totalUsd: z.number().positive(),
  messages: z.array(z.string().min(1)).min(2),
});
const intentGrowthOrderSignedSchema = z.object({
  orderId: z.string().min(1),
  buyerName: z.string().min(1),
  quantity: z.number().int().positive(),
  unitPriceUsd: z.number().positive(),
...
const intentGrowthAttestationIssuedSchema = z.object({
  attestationId: z.string().min(1),
  deliveredInDays: z.number().positive(),
  evidenceVerified: z.boolean(),
  slaHonored: z.boolean(),
  afterSalesExecutable: z.boolean(),
  trustDelta: z.number(),
  simulatedFulfillment: z.literal(true),
});
const intentGrowthRankUpdatedSchema = z.object({
  rankBefore: z.number().int().positive(),
  rankAfter: z.number().int().positive(),
  scoreBefore: z.number().min(0).max(100),
  scoreAfter: z.number().min(0).max(100),
  shortlistRateBefore: z.number().min(0).max(100),
  shortlistRateAfter: z.number().min(0).max(100),
  ordersPerDayBefore: z.number().min(0),
  ordersPerDayAfter: z.number().min(0),
  projectedNewIntents: z.number().int().min(0),
  simulatedProjection: z.literal(true),
});
...
export const agentEventSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.purchase.requested"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopPurchaseRequestSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.intent.structured"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopIntentSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.proposal.submitted"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopProposalSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.seller.rejected"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopSellerRejectedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.seller.selected"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopSellerSelectedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.counter.offer"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopCounterOfferSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.counter.response"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopCounterResponseSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.approval.requested"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopApprovalRequestedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.order.confirmed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopOrderConfirmedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.fulfillment.updated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopFulfillmentUpdatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("laptop.attestation.issued"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: laptopAttestationIssuedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.time.advanced"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockTimeAdvancedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.inventory.forecasted"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockInventoryForecastSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.authorization.checked"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockAuthorizationCheckSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.intent.created"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockIntentSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.proposal.submitted"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockProposalSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.seller.selected"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockSellerSelectedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.bundle.negotiated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockBundleNegotiatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.order.authorized"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockOrderAuthorizedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.order.confirmed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockOrderConfirmedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.inventory.updated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockInventoryUpdatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.memory.updated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockMemoryUpdatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("restock.notification.sent"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: restockNotificationSentSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.product.ingested"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesProductIngestedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.passport.published"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesPassportPublishedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.buyer.matched"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesBuyerMatchedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.proposal.routed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesProposalRoutedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.proposal.blocked"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesProposalBlockedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.buyer.selected"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesBuyerSelectedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("active-sale.completed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: activeSalesCompletedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("demand.need.received"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: demandNeedReceivedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("demand.intent.structured"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: demandIntentStructuredSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("demand.market.aggregated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: demandMarketAggregatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("demand.product.forecasted"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: demandProductForecastedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("demand.supply.negotiated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: demandSupplyNegotiatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("demand.batch.completed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: demandBatchCompletedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("distribution.contract.published"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: distributionContractPublishedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("distribution.agent.matched"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: distributionAgentMatchedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("distribution.orders.completed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: distributionOrdersCompletedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("distribution.commission.released"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: distributionCommissionReleasedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.market.ranked"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthMarketRankedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.seller.shortlisted"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthSellerShortlistedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.dialogue.round"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthDialogueRoundSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.seller.lost"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthSellerLostSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.learning.started"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthLearningStartedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.intent.extracted"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthIntentExtractedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.gap.detected"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthGapDetectedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.product.field.updated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthProductFieldUpdatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.product.version.published"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthProductVersionPublishedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.buyer.rematched"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthBuyerRematchedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.quote.requested"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthQuoteRequestedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.terms.negotiated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthTermsNegotiatedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.order.signed"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthOrderSignedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.attestation.issued"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthAttestationIssuedSchema,
  }),
  z.object({
    id: z.string().min(1),
    transactionId: z.string().min(1),
    type: z.literal("intent-growth.rank.updated"),
    source: z.string().min(1),
    target: z.string().optional(),
    timestamp: z.iso.datetime({ offset: true }),
    causationId: z.string().optional(),
    payload: intentGrowthRankUpdatedSchema,
  }),
]);
