  ...
  const { nodes, links } = useMemo(() => {
    ...
    if (mode === 'upstream') {
      const upstream = Object.values(agents).filter(
        (a) => a.role === 'supply' || a.role === 'factory' || a.id === seller.id,
      )
      const ids = new Set(upstream.map((a) => a.id))
      return {
        nodes: upstream,
        links: all.filter((l) => l.upstream && (ids.has(l.from) || ids.has(l.to))),
      }
    }
    // 下游获客：同品类卖家 + 该品类活跃买家
      ...
      <div className="mt-bar">
        ...
        <div className="mt-note">
          {mode === 'downstream'
            ...
            : '你的供应链 Agent 化身买家向上游采购。上游违约会沿链传导，拉低你对消费者的时效履约率。'}
        </div>
      </div>
      ...
      <div className="mt-stage">
        ...
        <div className="mt-rank panel">
          ...
          <div className="mt-myrank">
            你当前排名 <b className="num">#{myRank || '—'}</b> / {ranking.length}
          </div>
