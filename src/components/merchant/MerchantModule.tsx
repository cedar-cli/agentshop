    ...
    <div className="module merchant-module">
      <header className="merchant-bar">
        ...
        <div className="merchant-kpis">
          ...
          <span><b className="num">{seller.credit}</b>履约信用</span>
        ...
        {view === 'operations' && (
          <div className="merchant-operations">
            <nav className="operation-tabs">
              <OperationButton active={operation === 'fleet'} onClick={() => setOperation('fleet')} icon={<UsersRound size={14} />} label="员工编队" />
              <OperationButton active={operation === 'topology'} onClick={() => setOperation('topology')} icon={<Network size={14} />} label="上下游拓扑" />
              <OperationButton active={operation === 'credit'} onClick={() => setOperation('credit')} icon={<ShieldCheck size={14} />} label="履约信用" />
              <OperationButton active={operation === 'sandbox'} onClick={() => setOperation('sandbox')} icon={<SlidersHorizontal size={14} />} label="策略沙盒" />
              <OperationButton active={operation === 'eco'} onClick={() => setOperation('eco')} icon={<History size={14} />} label="生态演化" />
            </nav>
...
function OperationButton({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: ReactNode; label: string }) {
  return <button type="button" className={active ? 'on' : ''} onClick={onClick}>{icon}<span>{label}</span></button>
}
