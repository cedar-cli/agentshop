...
const PRESETS: Array<{ label: string; pref: OfferPreference }> = [
  {
    label: "最看重价格",
    pref: { timeliness: 10, spec: 10, price: 70, afterSales: 10 },
  },
  {
    label: "最看重时效",
    pref: { timeliness: 70, spec: 10, price: 10, afterSales: 10 },
  },
  {
    label: "最看重品质",
    pref: { timeliness: 10, spec: 70, price: 10, afterSales: 10 },
  },
  {
    label: "最看重售后",
    pref: { timeliness: 10, spec: 10, price: 10, afterSales: 70 },
  },
];

const METRIC_DIMS: Array<{
  key: keyof OfferMetrics;
  label: string;
  color: string;
}> = [
...
export function CounterfactualPanel({ purchase }: { purchase: DemoPurchase }) {
  ...
  const baselineChampion =
    purchase.offers.find((offer) => offer.selected) ?? null;
  const switched =
    !!currentChampion &&
    !!baselineChampion &&
  ...
  return (
    <div className="inspector-block cf-block">
            ...
            >
              ...
              <div className="cf-main">
                <div className="cf-name-row">
                  ...
                  {isChampion && (
                    <span className="cf-tag cf-tag-champ">当前冠军</span>
                  )}
      ...
      <p className="cf-foot">
        匹配分 = 你的偏好 ×
        卖家四维链上硬指标。公共信用只记录卖家有没有做到承诺； 个性化偏好由你的
        C-Agent 决策时自行加权，不写入公共信用。
      </p>
    </div>
  );
}
