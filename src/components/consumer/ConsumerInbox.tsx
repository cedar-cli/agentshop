...
export function ConsumerInbox({
  ...
  const selected =
    inbox.messages.find((item) => item.id === selectedId) ?? inbox.messages[0];

  useEffect(() => {
    if (!selectedId && inbox.messages[0]) setSelectedId(inbox.messages[0].id);
    if (
      selectedId &&
      !inbox.messages.some((item) => item.id === selectedId) &&
      inbox.messages[0]
    ) {
  ...
  const counts = useMemo(
    () => ({
      approval: inbox.messages.filter((message) => message.type === "approval")
        .length,
      completed: inbox.messages.filter(
        (message) => message.type === "completed",
      ).length,
      blocked: inbox.messages.filter((message) => message.type === "blocked")
        .length,
    }),
    [inbox.messages],
  );
  const visible =
    filter === "all"
      ? inbox.messages
      : inbox.messages.filter((message) => message.type === filter);
  ...
  if (traceTransactionId)
    return (
      <ConsumerTransactionTrace
        transactionId={traceTransactionId}
        onBack={() => setTraceTransactionId(undefined)}
      />
    );
  if (!selected)
    return (
  ...
  return (
    <div className="inbox-workspace">
      <aside className="inbox-list-panel">
        <header>
          <div>
            <span className="eyebrow">Agent 注意力队列</span>
            <h3>Inbox</h3>
          </div>
        ...
        <div className="inbox-api-state">
          <Radio size={11} />
          <span className={inbox.apiOnline ? "online" : "offline"}>
            {inbox.apiOnline ? "LIVE API" : "FIXTURE FALLBACK"}
          </span>
        </div>
        ...
        <div className="inbox-message-list">
          {visible.map((message) => (
            <button
              type="button"
              key={message.id}
              className={selected.id === message.id ? "on" : ""}
              onClick={() => setSelectedId(message.id)}
            >
              <span
                className={`inbox-verdict-dot verdict-${message.verdict}`}
              />
              <span>
                <strong>{message.title}</strong>
                <small>
                  {message.merchant} · {formatReceivedAt(message.receivedAt)}
                </small>
              </span>
      ...
      <section className="inbox-detail scroll-y">
        <header className="inbox-detail-head">
          <div>
            <span className="eyebrow">
              {selected.category} · {formatReceivedAt(selected.receivedAt)}
            </span>
            <h3>{selected.title}</h3>
            <p>{selected.merchant}</p>
          </div>
          <span className={`verdict-badge verdict-${selected.verdict}`}>
            {selected.verdictLabel}
          </span>
        </header>

        <div className="inbox-source-row">
          <span className={selected.runtime === "live" ? "live" : "fixture"}>
            {selected.runtime === "live" ? "LIVE EVENT" : "FIXTURE"}
          </span>
          <span>
            {selected.source === "active-service"
              ? "主动服务"
              : selected.source === "seller-agent"
                ? "Seller Agent"
                : "系统"}
          </span>
          <span>
            {selected.generatedBy === "llm" ? "LLM 解释" : "规则判定"}
          </span>
        ...
        <div className="merchant-offer-block">
          ...
          <div>
            <span>
              {selected.type === "completed" ? "执行结果" : "卖家提案"}
            </span>
        ...
        <div className={`agent-evaluation verdict-${selected.verdict}`}>
          <header>
            <Sparkles size={16} />
            <strong>Agent 评价</strong>
            <span className="num">价值 {selected.valueScore}/100</span>
          </header>
          <p>{selected.agentEvaluation}</p>
        </div>

        {selected.type === "completed" && (
          <div className="inbox-no-action">
            <Check size={13} />
            已自动处理，无需人工操作
          </div>
        )}
        {selected.transactionId ? (
          <button
            className="open-related"
            type="button"
            onClick={() => setTraceTransactionId(selected.transactionId)}
          >
            查看共享交易链
          </button>
        ) : (
          selected.relatedPurchaseId && (
            <button
              className="open-related"
              type="button"
              onClick={() => onOpenPurchase(selected.relatedPurchaseId!)}
            >
              {selected.type === "completed"
                ? "查看真实交易链"
                : "打开关联购买任务"}
        ...
        {(actionError || inbox.error) && (
          <div className="runtime-error">{actionError ?? inbox.error}</div>
        )}
      </section>
      ...
      <aside className="memory-decision-panel">
        ...
        <div className="memory-actions">
          <button
            type="button"
            className={selected.memoryRecommended ? "on" : ""}
          ...
          <button
            type="button"
            className={!selected.memoryRecommended ? "on blocked" : ""}
        ...
        {selected.senderPenaltyDays && (
          <div className="sender-penalty">
            <ShieldBan size={15} />
            发送方已降权 {selected.senderPenaltyDays} 天
          </div>
        )}
        <div className="memory-boundary">
          只保存可验证事实与稳定偏好，不保存广告措辞。
        </div>
      </aside>
    </div>
  );
