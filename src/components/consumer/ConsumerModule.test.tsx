...
describe("买家端演示工作台", () => {
  it("以会话 Agent 为入口并允许实时执行轻薄本采购", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /我的消费 Agent/ }),
    ).toBeInTheDocument();
    expect(screen.getByText("委托与历史")).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /运行真实 LLM/ }),
    ).toBeInTheDocument();
  ...
  it("家庭补库展示自主监测而不是购买确认", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /推进到库存触发点/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: "向消费 Agent 描述需求" }),
    ).toBeDisabled();
    expect(
      screen.queryByRole("button", { name: "确认下单" }),
    ).not.toBeInTheDocument();
  });

  it("新建委托入口提供主动服务方式选择与自由意图输入", async () => {
    const user = userEvent.setup();
    render(<ConsumerModule />);
    await user.click(screen.getByRole("button", { name: /新建委托/ }));
    expect(
      screen.getByRole("heading", { name: "新增委托任务" }),
    ).toBeInTheDocument();
    const composer = screen.getByRole("textbox", { name: "新增委托任务" });
    await user.type(composer, "@");
    expect(screen.getByText("主动服务方式")).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: /高值蹲低价/ }),
    ).toBeInTheDocument();
  });

  it("发起委托后由消费 Agent 全自动接管并生成委托会话", async () => {
    const user = userEvent.setup();
    render(<ConsumerModule />);
    await user.click(screen.getByRole("button", { name: /新建委托/ }));
    const composer = screen.getByRole("textbox", { name: "新增委托任务" });
    await user.type(composer, "买一台安静的加湿器，预算 400");
    await user.click(screen.getByRole("button", { name: "发起委托" }));
    expect(
      await screen.findByText("AUTONOMOUS DELEGATION"),
    ).toBeInTheDocument();
    expect(screen.getByText("委托任务 · 真实 LLM")).toBeInTheDocument();
  });
