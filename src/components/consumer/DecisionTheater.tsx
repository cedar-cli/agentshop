      ...
      <header className="theater-head">
        ...
        <div className="theater-controls">
          <button type="button" className="theater-play" onClick={toggle}>
            ...
            <span>{playing ? '暂停' : arenaDone ? '重新连演' : '连演全程'}</span>
          </button>
          <button type="button" className="theater-reset" onClick={reset} title="重置">
            <RotateCcw size={13} />
          </button>
        </div>
      </header>
    ...
    <div className="lineup">
      <div className="lineup-status">
        ...
        >
          {current ? (
            <>
              {current.status === 'pass' ? <Check size={14} /> : <X size={14} />}
              <b>{current.seller}</b>
        ...
        {LINEUP_POOL.map((candidate, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="lineup-chip-credit num">信 {candidate.credit}</span>
  ...
  return (
    <div className="arena">
        ...
        <div className="arena-center">
          ...
          {done && (
            <div className="arena-deal">
              <Gavel size={13} />成交 · 省 {yuan(saved)} + 延保 1 年
            </div>
          )}
        </div>
            ...
            >
              <div className="arena-card">
                <div className="arena-card-head">
                  <span className="arena-card-type">{CARD_LABEL[round.cardType]}</span>
      ...
      <p className="arena-foot">
        筹码 = 链上信用硬数据，不是话术。买家甩证据、卖家亮履约，价格随可信数据收敛。
      </p>
    </div>
  )
}
