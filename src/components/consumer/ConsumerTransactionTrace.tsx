...
function eventLabel(type: string) {
  const labels: Record<string, string> = {
    ...
    'laptop.proposal.submitted': '商家提交报价',
...
function payloadSummary(event: StoredEvent) {
  ...
  if (typeof value.productName === 'string') return `${value.productName} · ${String(value.amountUsd ?? '')}`
...
export function ConsumerTransactionTrace({ transactionId, onBack }: { transactionId: string; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/transactions/${transactionId}`)
      ...
      .then((value) => { if (active) { setSnapshot(value); setEvents(value.events) } })
    ...
    const eventTypes = [
      'active-sale.product.ingested', 'active-sale.passport.published', 'active-sale.buyer.matched',
      'active-sale.proposal.routed', 'active-sale.proposal.blocked', 'active-sale.buyer.selected', 'active-sale.completed',
      'laptop.intent.structured', 'laptop.proposal.submitted', 'laptop.seller.selected', 'laptop.order.confirmed',
      'restock.intent.created', 'restock.order.confirmed',
    ]
  ...
  return <div className="consumer-transaction-trace">
    <header>
      <button type="button" onClick={onBack} title="返回 Inbox"><ArrowLeft size={15} /></button>
      <div><span className="eyebrow">SHARED TRANSACTION</span><h3>消费者与商家共享事件链</h3><code>{transactionId}</code></div>
      <span className="trace-chain"><ShieldCheck size={14} />{snapshot?.chainValid === false ? 'CHAIN ERROR' : 'HASH CHAIN'}</span>
    </header>
    {error && <div className="runtime-error">{error}</div>}
    <div className="trace-runtime"><Radio size={13} /><span>{snapshot?.status ?? 'running'}</span><b>{events.length} events</b></div>
    <ol>
      {events.map((event) => <li key={event.id}>
        <span><Bot size={13} /></span>
        <div><header><strong>{eventLabel(event.type)}</strong><time>{new Date(event.timestamp).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false })}</time></header><p>{payloadSummary(event)}</p><small>{event.source}{event.target ? ` → ${event.target}` : ''}</small></div>
      </li>)}
    </ol>
  </div>
}
