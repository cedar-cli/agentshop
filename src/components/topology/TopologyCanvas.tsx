    ...
    const render = () => {
      ...
      for (const e of linksRef.current) {
        ...
        if (!bidding) {
          ...
          ctx.beginPath()
          ctx.fillStyle = color
          ctx.arc(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, inFocus ? 3 : 2, 0, 7)
          ctx.fill()
        }
      }

      // 节点（记录需要显示名称的活跃/焦点节点，最后统一绘制标签，保证在最上层）
      const labels: Array<{ x: number; y: number; text: string; color: string; strong: boolean }> = []
      for (const n of nodesRef.current) {
        ...
        if (!idle && !dim && (isFocus || (!!active && active.has(n.id)) || isHover)) {
          labels.push({ x: p.x, y: p.y - r - 6, text: n.name, color: base, strong: isFocus })
        }
      }
