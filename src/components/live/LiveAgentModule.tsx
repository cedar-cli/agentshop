...
function LiveButton({
  ...
  return (
    ...
    >
      {busy ? (
        <Activity size={15} className="live-spin" />
      ) : done ? (
        <RefreshCcw size={15} />
      ) : (
        <Play size={15} />
...
export function LiveAgentModule({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div
      className={
        embedded ? "live-module live-module-embedded" : "module live-module"
      }
    >
      <header className="live-head">
        <div className="live-title-wrap">
          <span className="live-agent-mark">
            <Bot size={18} />
          </span>
          <div>
            <h2 className="module-title">实时 Agent 交易</h2>
            <p className="module-desc">
              真实 Router、OpenAI 模型、SSE
              事件与哈希链共同驱动，不使用前端伪事件。
            </p>
          </div>
        </div>
        <div className="live-runtime panel-inset">
          <span className={runtime?.llmConfigured ? "online" : "offline"}>
            <Radio size={13} /> API
          </span>
          <strong>{runtime?.model ?? "连接中"}</strong>
          <span>
            {runtime?.evidenceLlmEnabled
              ? "Evidence LLM ON"
              : "Safe fallback ready"}
          </span>
        </div>
...
function PurchasePanel() {
  ...
  const selection = payload<SellerSelectionPayload>(
    latestEvent(demo.events, "seller.selected"),
  );
  const counterOffer = payload<CounterOfferPayload>(
    latestEvent(demo.events, "counter.offer"),
  );
  const counterResponse = payload<CounterResponsePayload>(
    latestEvent(demo.events, "counter.response"),
  );
  const order = payload<ConfirmedOrderPayload>(
    latestEvent(demo.events, "order.confirmed"),
  );
  ...
  return (
    <div className="live-grid live-purchase-grid">
      <section className="live-column live-intent-column">
        ...
        <LiveButton
          busy={busy}
          done={demo.phase === "completed"}
          onStart={start}
        />
        <div className="live-tx">
          <span>TX</span>
          <code>{demo.transactionId ?? "尚未创建"}</code>
        </div>
      ...
      <section className="live-column live-market-column">
        <div className="live-section-title">
          <span className="eyebrow">SELLER PROPOSALS</span>
          <h3>三个 Seller Agent 实时报价</h3>
        </div>
        <div className="live-proposals">
          {proposals.length === 0 && (
            <EmptyState text="发布需求后，三个卖家 Agent 将同时调用模型生成报价。" />
          )}
          {proposals.map((proposal) => (
            <article
              key={proposal.sellerId}
              className={`live-proposal ${selection?.sellerId === proposal.sellerId ? "winner" : ""}`}
            >
              <header>
                <strong>
                  {SELLER_NAMES[proposal.sellerId] ?? proposal.sellerId}
                </strong>
                <SourceBadge source={proposal.generatedBy} />
              </header>
              <div className="live-metrics">
                <span>
                  <b>${proposal.totalPrice.toFixed(2)}</b>报价
                </span>
                <span>
                  <b>{proposal.deliveryHours}h</b>交付
                </span>
                <span>
                  <b>{proposal.reputation}</b>信用
                </span>
              </div>
      ...
      <section className="live-column live-decision-column">
        ...
        {!counterOffer && (
          <EmptyState text="买家完成全量比较后，自动向赢家发起还价。" />
        )}
        {counterOffer && (
          <div className="live-dialogue">
            <div className="buyer">
              <span>C-Agent</span>
              <p>{counterOffer.reasoning}</p>
              <b>
                ${counterOffer.originalPrice.toFixed(2)} → $
                {counterOffer.targetPrice.toFixed(2)}
              </b>
            </div>
            {counterResponse && (
              <div className="seller">
                <span>S-Agent · {counterResponse.sellerId}</span>
                <SourceBadge source={counterResponse.generatedBy} />
                <p>{counterResponse.reasoning}</p>
                <b>
                  最终成交 ${counterResponse.finalPrice.toFixed(2)} · 让利 $
                  {counterResponse.concession.toFixed(2)}
                </b>
              </div>
            )}
          </div>
        ...
        {order && (
          <div className="live-order">
            ...
            <div>
              <span>Agent Order Confirmed</span>
...
function EvidencePanel({
  ...
  scenario: NewbornScenario | undefined;
}) {
  const demo = useTransactionStream(EVIDENCE_EVENTS, "receipt.issued");
  const busy = demo.phase === "queued" || demo.phase === "running";
  const sellerCSubmission = payload<EvidenceSubmissionPayload>(
    latestEvent(demo.events, "evidence.submitted", "seller-c"),
  );
  const authorization = payload<AuthorizedOrderPayload>(
    latestEvent(demo.events, "order.authorized"),
  );
  const receipt = payload<ReceiptPayload>(
    latestEvent(demo.events, "receipt.issued"),
  );
  ...
  const act = receipt
    ...
    : sellerCSubmission
      ? 3
      : demo.events.some((event) => event.type === "seller.matched")
        ? 2
        : demo.events.some((event) => event.type === "intent.published")
          ? 1
          : 0;
  ...
  return (
    <div className="live-evidence-layout">
      <div className="live-actbar">
        {["发布意图", "卖家匹配", "机器询证", "自动购买"].map(
          (label, index) => (
            <span key={label} className={act >= index + 1 ? "on" : ""}>
              <b>{index + 1}</b>
              {label}
            </span>
          ),
      ...
      <div className="live-grid live-evidence-grid">
        <section className="live-column live-intent-column">
          ...
          <div className="live-intent-card panel-inset">
            <strong>
              {scenario?.intent.productDescription ?? "场景加载中"}
            </strong>
            <dl>
              <div>
                <dt>预算</dt>
                <dd>${scenario?.intent.budgetUsd ?? 180}</dd>
              </div>
              <div>
                <dt>时限</dt>
                <dd>{scenario?.intent.deadlineHours ?? 72} 小时</dd>
              </div>
              <div>
                <dt>风险阈值</dt>
                <dd>&lt; {scenario?.intent.riskThreshold ?? 0.15}</dd>
              </div>
              <div>
                <dt>授权</dt>
                <dd>满足证据后自动购买</dd>
            ...
            <ul>
              {scenario?.intent.unacceptable.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>
          <LiveButton
            ...
            onStart={() =>
              demo.start({ endpoint: "/api/demo/newborn-bedding" })
            }
          />
          <div className="live-tx">
            <span>TX</span>
        ...
        <section className="live-column live-market-column">
          <div className="live-section-title">
            <span className="eyebrow">DECISION RANKING</span>
            <h3>证据驱动排名</h3>
          </div>
          <div className="live-ranking">
            {scores.length === 0 && (
              <EmptyState text="意图广播后，三家卖家将提交证据并重新排名。" />
            )}
            {scores.map((score) => (
              <article
                key={score.sellerId}
              ...
              >
                ...
                <div>
                  <strong>
                    {scenario?.sellers.find(
                      (seller) => seller.sellerId === score.sellerId,
                    )?.displayName ?? score.sellerId}
                  </strong>
                ...
                <dl>
                  ...
                  <span>Delivery {score.deliveryConfidence}</span>
        ...
        <section className="live-column live-decision-column">
          <div className="live-section-title">
            <span className="eyebrow">SELLER C EVIDENCE</span>
            <h3>真实模型询证回答</h3>
          </div>
          {!sellerCSubmission && (
            <EmptyState text="Seller C 收到问题后，将使用模型在可验证事实边界内作答。" />
          )}
          {sellerCSubmission && (
            <div className="live-evidence-answers">
              <header>
                <strong>Seller C 回答来源</strong>
                <SourceBadge source={sellerCSubmission.generatedBy} />
              </header>
              {Object.entries(sellerCSubmission.answers).map(
                ([question, answer]) => (
                  <div key={question}>
                    <span>
                      {question.replace(/^q-/, "").replaceAll("-", " ")}
                    </span>
                    <p>{answer}</p>
                  </div>
                ),
          ...
          {authorization && receipt && (
            <div className="live-receipt">
              <HandCoins size={21} />
              <div>
                <span>Buyer Agent authorized purchase</span>
                <strong>${receipt.amountUsd}</strong>
                <small>{receipt.deliveryHours}h delivery · auto-approved</small>
              </div>
...
function EventStrip({
  ...
  return (
    <div className="live-event-strip">
      ...
      <code>
        {events.length ? events[events.length - 1]?.type : "Router waiting"}
      </code>
      {chainValid !== undefined && (
        <b className={chainValid ? "valid" : "invalid"}>
