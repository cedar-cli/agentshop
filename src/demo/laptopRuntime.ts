...
const typedPayload = <T,>(event: StoredEvent | undefined) => event?.payload as T | undefined
const latest = (events: StoredEvent[], type: string) => [...events].reverse().find((event) => event.type === type)
const origin = (generatedBy?: 'llm' | 'fallback'): DemoEvent['origin'] => generatedBy === 'llm' ? 'llm' : 'rule'
...
export function adaptLaptopPurchase(base: DemoPurchase, events: StoredEvent[]): DemoPurchase {
  if (events.length === 0) return base
  const intent = typedPayload<LaptopIntentPayload>(latest(events, 'laptop.intent.structured'))
  const selection = typedPayload<LaptopSelectionPayload>(latest(events, 'laptop.seller.selected'))
  const counter = typedPayload<LaptopCounterPayload>(latest(events, 'laptop.counter.response'))
  const order = typedPayload<{ totalPriceCny: number }>(latest(events, 'laptop.order.confirmed'))
  const approval = latest(events, 'laptop.approval.requested')
  const completed = events.some((event) => event.type === 'laptop.attestation.issued')
  ...
  const offers: DemoOffer[] = proposals.map((proposal) => ({
    ...
    price: selection?.sellerId === proposal.sellerId && paid > 0 ? paid : proposal.quotedPriceCny,
    originalPrice: proposal.quotedPriceCny,
    credit: proposal.reputation,
    match: selection?.sellerId === proposal.sellerId ? Math.round(selection.score) : Math.round(
      (proposal.metrics.timeliness + proposal.metrics.spec + proposal.metrics.price + proposal.metrics.afterSales) / 4,
    ),
  ...
  return {
    ...
    paid,
    saved: paid > 0 ? Math.max(0, (intent?.budgetCny ?? base.budget) - paid) : 0,
  ...
  switch (event.type) {
    ...
    case 'delegation.search.completed': {
      ...
      const payload = typedPayload<{
        ...
        hitCount: number
        hits: Array<{ title: string; shopName: string; priceMin: number; priceMax: number; image?: string }>
      }>(event)!
    ...
    case 'laptop.intent.structured': {
      ...
      return { ...common, kind: 'agent', actor: 'C-Agent', title: '约束结构化', body: `预算 ¥${payload.budgetCny.toLocaleString()}；重量 ≤${payload.maxWeightKg}kg；续航 ≥${payload.minBatteryHours}h；${Math.ceil(payload.deadlineHours / 24)} 天内送达。`, evidence: '由模型提取后通过服务端 Schema 和合理区间校验。', origin: origin(payload.generatedBy) }
    }
    case 'laptop.proposal.submitted': {
      const payload = typedPayload<LaptopProposalPayload>(event)!
      return { ...common, kind: 'seller-message', actor: `${payload.displayName} S-Agent`, title: '提交受约束报价', body: `${payload.reasoning} 报价 ¥${payload.quotedPriceCny.toLocaleString()}，${eta(payload.deliveryHours)}，${payload.warrantyYears} 年联保。`, origin: origin(payload.generatedBy) }
    }
    case 'laptop.seller.rejected': {
      const payload = typedPayload<{ displayName: string; reasons: string[] }>(event)!
      return { ...common, kind: 'evidence', actor: 'C-Agent', title: `${payload.displayName} 未通过硬约束`, body: payload.reasons.join('；'), impact: '候选淘汰', origin: 'rule' }
    }
    case 'laptop.seller.selected': {
      const payload = typedPayload<LaptopSelectionPayload>(event)!
      return { ...common, kind: 'comparison', actor: 'C-Agent', title: '全量比较并选标', body: `${payload.displayName}综合匹配 ${payload.score} 分。${payload.reason}`, evidence: '确定性权重计算，不由模型直接决定赢家。', origin: 'rule' }
    }
    case 'laptop.counter.offer': {
      const payload = typedPayload<{ originalPriceCny: number; targetPriceCny: number; reasoning: string }>(event)!
      return { ...common, kind: 'negotiation', actor: 'C-Agent', title: '发起自动议价', body: payload.reasoning, impact: `¥${payload.originalPriceCny.toLocaleString()} → 目标 ¥${payload.targetPriceCny.toLocaleString()}`, origin: 'rule' }
    }
    case 'laptop.counter.response': {
      const payload = typedPayload<LaptopCounterPayload>(event)!
      return { ...common, kind: 'negotiation', actor: '中标 S-Agent', title: '商家回应议价', body: payload.reasoning, impact: `最终 ¥${payload.finalPriceCny.toLocaleString()} · 让利 ¥${payload.concessionCny.toLocaleString()}`, origin: origin(payload.generatedBy) }
    }
    case 'laptop.approval.requested': {
      const payload = typedPayload<{ reason: string; expiresInMinutes: number }>(event)!
