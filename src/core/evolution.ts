/* ============================================================
   ...
   ------------------------------------------------------------
   消费者 Agent（带 persona 画像与真实意图输入）持续向商家发起需求，
   经匹配 → 沟通 → 成交 → 履约，产生「满意 / 差评 / 流失」结果；
   每个结果按维度归因（描述/服务/时效/选品/推广/价格），
   累计驱动商家信誉与排名变化，并生成诊断报告与改进建议。
...
export const FACTORS: Array<{
...
}> = [
  {
    ...
    label: '商品描述',
    issue: '描述与实物不符',
    advice: '校准标称参数，让描述贴近实物抽检值，杜绝夸大宣传，减少参数不符差评。',
  },
  {
    key: 'service',
    label: '售后服务',
...
const FACTOR_MAP: Record<FactorKey, (typeof FACTORS)[number]> = Object.fromEntries(
  FACTORS.map((f) => [f.key, f]),
) as Record<FactorKey, (typeof FACTORS)[number]>
...
function buildFactors(rng: Rng, credit: number, weak: FactorKey | null): Record<FactorKey, number> {
...
export function createSim(
  ...
  const merchants: EcoMerchant[] = inCategory.map((seller, i) => {
    ...
    return {
      ...
      category,
      archetype,
      factors: buildFactors(rng, seller.credit, weak),
...
function experience(m: EcoMerchant, persona: EcoPersona): number {
  ...
  const specAbility = (m.factors.listing + m.factors.selection) / 2
  return (
    (p.timeliness * m.factors.fulfilment +
      p.spec * specAbility +
      p.price * m.factors.price +
      p.afterSales * m.factors.service) /
    wSum
  )
}
...
export function stepSim(sim: EcoSim): EcoSim {
  ...
  const merchants = sim.merchants.map((m) => ({ ...m, factors: { ...m.factors }, issues: { ...m.issues } }))
  ...
  for (let i = 0; i < DEALS_PER_STEP; i++) {
    ...
    // 机会流失：本该拿下这个 persona、却因某项短板出局的商家。
    // 归因到「买家在意 + 商家最差」的维度，让每种短板都能通过流失暴露出来。
    if (scored.length > 1) {
      ...
      if (laggard && laggard.deficit.gap > LOST_GAP_THRESHOLD) {
        ...
        laggard.m.credit = clamp(laggard.m.credit - 0.9)
...
export function diagnose(m: EcoMerchant, rank?: number): EcoDiagnosis {
  ...
  const trend =
    m.credit >= m.startCredit ? '信用上行' : '信用下滑'
