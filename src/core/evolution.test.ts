...
describe("evolution · 事件驱动交易模拟", () => {
  ...
  it.each(expected)("归因正确：%s 的主要问题是 %s", (archetype, factor) => {
    const sim = runYear(42);
  ...
  it("intentBoards：每个意图各一张榜，名次连续，且不同意图排序不完全相同", () => {
    const sim = runYear(42);
    const boards = intentBoards(sim);
    expect(boards).toHaveLength(PERSONAS.length);
    for (const b of boards) {
      expect(b.rows).toHaveLength(6);
      expect(b.rows.map((r) => r.rank)).toEqual([1, 2, 3, 4, 5, 6]);
      // 名次变化 = 初始名次 − 当前名次
      for (const r of b.rows) expect(r.delta).toBe(r.startRank - r.rank);
    }
    // 「多个意图榜」的价值：至少两张榜的排序不同
    const orders = boards.map((b) => b.rows.map((r) => r.merchantId).join(","));
    expect(new Set(orders).size).toBeGreaterThan(1);
  });

  it("intentDiagnosis：短板卖家在不同意图榜名次悬殊，给出跨意图对比建议", () => {
    const sim = runYear(42);
    const slow = byArchetype(sim, "slow");
    const d = intentDiagnosis(sim, slow.id);
    expect(d.standings).toHaveLength(PERSONAS.length);
    expect(d.best).not.toBeNull();
    expect(d.worst).not.toBeNull();
    // 同一履约，擅长与垫底的意图榜名次拉开
    expect(d.best!.rank).toBeLessThan(d.worst!.rank);
    // 建议点名擅长与垫底的意图榜
    expect(d.advice).toContain(d.best!.label);
    expect(d.advice).toContain(d.worst!.label);
  });

  it("优等生在意图榜整体领先，跨意图建议为空（无明显短板）", () => {
    const sim = runYear(42);
    const diligent = byArchetype(sim, "diligent");
    const d = intentDiagnosis(sim, diligent.id);
    // 各榜名次均衡时不产出对比建议
    if (d.best!.rank === d.worst!.rank) expect(d.advice).toBe("");
  });

  it("ranking：风控降级的卖家沉底", () => {
    const sim = runYear(42);
    const board = ranking(sim);
